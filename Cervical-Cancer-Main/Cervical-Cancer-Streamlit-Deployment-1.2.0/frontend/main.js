(() => {
  ...
  function setFrameHeight() {
    // Match the visible viewport instead of full content scrollHeight so the
    // outer Streamlit page never has to grow taller than the screen and
    // show its own scrollbar. Any internal overflow is handled (and hidden)
    // inside .stage via CSS.
    const viewportHeight = window.innerHeight || (window.screen && window.screen.availHeight) || 800;
    sendMessage("streamlit:setFrameHeight", { height: viewportHeight });
  }
