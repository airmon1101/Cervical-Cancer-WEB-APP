...
# --------------------------------------------------------------------- scan
with col_scan:
    st.markdown('<div class="cyto-panel">', unsafe_allow_html=True)
    st.markdown('<div class="cyto-panel-label">01 · SPECIMEN</div>', unsafe_allow_html=True)
...
with col_read:
    st.markdown('<div class="cyto-panel">', unsafe_allow_html=True)
    st.markdown('<div class="cyto-panel-label">02 · READOUT</div>', unsafe_allow_html=True)

    result = st.session_state.result
    if result is None:
        st.markdown(
            '<div class="cyto-readout-empty">Results appear here once a specimen is analyzed.</div>',
            unsafe_allow_html=True,
        )
    else:
        label = result["label"]
